    ...
    public class RendererSystem : DestroySystem
    {
        ...
        public override void Update()
        {
            ...
            for (int j = 0; j < renderers.GetLength(1); j++)
            {
                for (int i = 0; i < renderers.GetLength(0); i++)
                {
                    list.Add(renderers[i, j]);
                }
            }

            #region 废弃,兼容windows的格式,先不管,换成更舒服的格式
            /*
            //不对 问题不在于没有双缓冲,这里虽然没有双缓冲了,但是Winform做了双缓冲处理.唯一的一点问题就是winform不需要进行拆分两部分
            //可能考虑的优化手段  1 这个取消分割投送模式,但是要封装保留,留给以后的consoleoutput.
            //2. 尝试异步处理gdi,看看能不能有改善. 当update到
            List<RenderPoint> list = new List<RenderPoint>();
