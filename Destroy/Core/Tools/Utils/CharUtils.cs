namespace Destroy
{
    ...
    public static class CharUtils
    {
        ...
        /// <summary>
        /// 按照标准长度截断数字作为字符串,不足用空格补上(使用左补空格),超出截断
        /// </summary>
        public static string ParseNum(int num, int width)
        {
            string str = num.ToString();

            int needAppendBefore = width - str.Length;
            if (needAppendBefore < 0)
                needAppendBefore = 0;

            StringBuilder builder = new StringBuilder();
            //长度不足用空格来补
            for (int i = 0; i < needAppendBefore; i++)
            {
                builder.Append(' ');
            }

            for(int i = 0; i< width - needAppendBefore;i++)
            {
